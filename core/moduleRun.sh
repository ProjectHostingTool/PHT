modulename="$2"
params=("$@")
[[ -z $modulename ]] && errorlog "You have to set module name!" && exit 1
...
[[ $(docker ps -as | grep "$id") =~ ("up"|"UP"|"Up") ]] && infolog "System already running."  && sublog "if you want to connect use '--console' parameter." && exit 0
