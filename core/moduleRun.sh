...
[[ $(docker ps -as | grep "$id" | awk '{print $1}') != "$id" ]] && log.error "Docker ID not match!"   && log.submessage "Reference ID -> $id" && exit 1

if [[ "$3" =~ ("-t"|"--terminal"|"terminal"|"-c"|"--console"|"console") ]];then 
    [[ -z "${params[@]:3}" ]] && log.warn "You must set arguments." && log.submessage "Ex: bash, cat <args>" && exit 1
...
! [[ -f "core/modules/$name/$exec" ]] && log.error "Module exec file not found!" && log.submessage "/opt/PHT/core/modules/$modulename/$exec" && exit 1
