...
rm core/modules/confs/$modulename.conf || { stopanimation "error" && log.error "Conf can not removed!" && log.submessage "PATH -> /opt/PHT/core/modules/confs/$modulename.conf" && exit 1; }
rm -r core/modules/$modulename || { stopanimation "error" && log.error "Module Path can not removed!" && log.submessage "PATH -> /opt/PHT/core/modules/$modulename" && exit 1; }
docker remove $id &>/dev/null || { stopanimation "error" && log.error "Container can not removed!" && log.submessage "Reference ID -> $id" && exit 1; }
stopanimation "done"
